...
export default {
    ...
    methods: {
        countMax(max) {
            ...
            let maxNum =
                Math.ceil(parseFloat(num / parseInt(base))) * parseInt(base);
            if ((maxNum / parseInt(base)) % 3 != 0) {
                return (
                    Math.ceil(maxNum / parseInt(base) / 3) * 3 * parseInt(base)
                );
            }
            return maxNum;
        },
                ...
                option.dataZoom = [
                    {
                        ...
                        handleSize: 0,
                        zoomLock: true,
                        fillerColor: this.zoomFillCrl
                            ? this.zoomFillCrl
                            : "#114c91",
                        backgroundColor: this.zoomBgcCrl
                            ? this.zoomBgcCrl
                            : "#434651",
                        borderColor: this.zoomBorCrl
                            ? this.zoomBorCrl
                            : "rgba(0,0,0,0)",
                        showDetail: false,
